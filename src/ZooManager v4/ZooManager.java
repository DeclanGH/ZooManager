...
public class ZooManager {
    ...
    private static int doBinarySearch(ArrayList<String> idToArray, String selectedID) {
        // selectedID is our target and our array has been created
        int lowIndex = 0;
        int highIndex = idToArray.size()-1;
        while (lowIndex<=highIndex){
            int midIndex = (lowIndex+highIndex)/2;
            Benchmark.increaseCounterBS(); // Increase counter after the array is split
            if(idToArray.get(midIndex).equals(selectedID)){
                return Benchmark.getCounterBS(); // If found, return our counter value
            }
            if(idToArray.get(midIndex).compareTo(selectedID)<0){ // compareTo method for strings
                lowIndex = midIndex + 1;
            }else highIndex = midIndex - 1;
        }
        return Benchmark.getCounterBS(); // Need this here as a return statement.
           /*Normally it'll return -1 if the ID isn't found, but this is an errorless code
           The "contains" method in the runFindMenu ensures the ID is always in the array, else an error message*/
    }

    private static int doLinearSearch(ArrayList<String> idToArray, String selectedID) {
        // Basic Linear Search.
        for(int i=0; i<idToArray.size(); i++){
            Benchmark.increaseCounterLS(); // Every iteration would increase our counter
            if(idToArray.get(i).equals(selectedID)){
             return Benchmark.getCounterLS(); // Code ends when ID is found and returns the counter value
            }
        }
        return Benchmark.getCounterLS(); // Return Statement
    }

    private static void runFindMenu(JsonObject obj) {
        System.out.print("Enter Animal ID you want to find >>>");
        iDFinder.useDelimiter("\n");
        String inputFormat = iDFinder.next();
        String selectedID = inputFormat.substring(inputFormat.indexOf(" ")+1);
        ...
        if (idToArray.contains(selectedID)){
            /* The above if statement would not let a wrong ID get to our binary and linear search method.
             This means that the target MUST exist in our array, else the error message below.*/
            ...
            int binaryBenchMark = doBinarySearch(idToArray,selectedID);
            int linearBenchMark = doLinearSearch(idToArray,selectedID);
            System.out.println("  [BENCHMARK RESULT]");
            System.out.println(linearBenchMark+" comparisons by Linear Search, "+
                    binaryBenchMark+ " comparisons by Binary Search");
            // The below methods resets the counter for our benchmarks to zero.
